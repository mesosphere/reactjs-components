...
class Docs extends Util.mixin(BindMixin) {
  ...
  handleNavigationClick(id) {
    let {pageRef} = this.state;
    let sectionPosition = this.sectionScrollPositions[id];
  ...
  handlePageScroll() {
    ...
    let pageScrollTop = this.state.pageRef.scrollTop;
    let scrollThreshhold = pageScrollTop + this.viewportHeight / 2;

    Object.keys(this.sectionScrollPositions).forEach((ref) => {
      let nodePosition = this.sectionScrollPositions[ref];
  ...
  calculateNodePositions() {
    let pageHeaderHeight = this.nodeRefs.pageHeader.offsetHeight;
    this.viewportHeight = DOMUtil.getViewportHeight();

    Object.keys(this.nodeRefs.sectionRefs).forEach((ref) => {
      let top = this.nodeRefs.sectionRefs[ref].offsetTop + pageHeaderHeight;
      this.sectionScrollPositions[ref] = top;
    });
  }

  getNavigationItems() {
    let items = navigationItems.map((item, index) => {
  ...
  render() {
    ...
    if (pageRef !== null) {
      docsContent = navigationItems.map((navigationItem, index) => {
        let {id, passScrollContainer} = navigationItem;
        let props = {};
        ...
        return (
          <div key={index} ref={(ref) => {this.nodeRefs.sectionRefs[id] = ref}}>
    ...
    return (
      <div id="canvas" className="canvas flex flex-direction-top-to-bottom flex-direction-left-to-right-screen-medium flex-item-grow-1">
        ...
        <div
          ...
          <div id="page-header" className="page-header fill fill-light left"
              ...
              <div className="container-fluid pod">
                <div className="row">
                  <div className="column-small-8 column-medium-8 column-large-10">
                    <h1 className="page-header-headline flush-top text-align-left short">ReactJS Components</h1>
                    <p className="page-header-description lead text-align-left flush-bottom">A library of reusable React components.</p>
                  </div>
                </div>
              </div>
              <a href="https://github.com/mesosphere/reactjs-components" className="github-corner">
                <svg className="octocat" viewBox="0 0 150 125">
                  <path className="octocat-tail" d="M63.71,111.33c-16.83,3.68-20.29-7.14-20.29-7.14-2.76-7-6.72-8.84-6.72-8.84-5.59-3.75.42-3.68,0.42-3.68,6,0.49,9.26,6.29,9.26,6.29,5.44,9.12,13.93,6.72,17.54,4.95" />
                  <path className="octocat-body" d="M58.55,125c0,0.14,3.68-1.56,3.68-3.11l0.07-19.59c0.57-4,2.12-6.65,3.89-8.13C52.75,92.6,38.68,87.43,38.61,64.31A23.51,23.51,0,0,1,44.9,48.12c-0.71-1.56-2.76-7.71.57-16,0,0,5-1.63,16.62,6.15a58.66,58.66,0,0,1,15.06-2.05,54.32,54.32,0,0,1,15.13,2.12c11.53-7.85,16.62-6.29,16.62-6.29,3.25,8.34,1.2,14.5.57,16a23.44,23.44,0,0,1,6.22,16.26c0,23.19-14.14,28.28-27.58,29.84,2.12,1.84,4.1,5.52,4.1,11.17L92.13,121.8c0,1.7,4.24,3.39,4.31,3.18H58.55Z"/>
                </svg>
              </a>
            </div>
          </div>
          <div id="page-content" className="page-content flex flex-direction-top-to-bottom flex-item-grow-1">
            <div className="container-fluid pod">
              <section id="goals">
                <h3 className="flush-top">Goals</h3>
                <ul>
                  <li><strong>Independent</strong> &mdash; pick and use only the components you need.</li>
                  <li><strong>Styled</strong> &mdash; default classes from <a href="http://mesosphere.github.io/canvas/">Canvas UI</a> (use is optional).</li>
                  <li><strong>Customizable</strong> &mdash; properties allow many different settings to adapt to your app.</li>
                  <li><strong>Reliable</strong> &mdash; each component is rigorously tested.</li>
                </ul>
              </section>
              <section id="getting-started">
                <h3>Getting Started</h3>
                <p><strong>1.</strong> From the command line inside of your project:</p>
                <div className="panel pod flush-right flush-left flush-top">
                  <div className="panel-cell panel-cell-narrow panel-cell-short panel-cell-light panel-cell-code-block">
                    <pre className="prettyprint transparent flush lang-shell">npm install --save reactjs-components react react-gemini-scrollbar canvas-ui</pre>
                  </div>
                </div>
                <p><strong>2.</strong> Import the component that you want to use:</p>
                <div className="panel pod flush-right flush-left flush-top">
                  <div className="panel-cell panel-cell-narrow panel-cell-short panel-cell-light panel-cell-code-block">
  ...
  return (
    <Modal ...{props}>
      // Content
                ...
                <p><strong>4.</strong> Import LESS files which will add all styles for all components.</p>
                <div className="panel pod flush-right flush-left flush-top">
                  <div className="panel-cell panel-cell-narrow panel-cell-short panel-cell-light panel-cell-code-block">
                    <pre className="prettyprint transparent flush lang-less">@import "./node_modules/canvas-ui/styles/canvas.less"
  @import (inline) "./node_modules/reactjs-components/lib/index.less"</pre>
                  </div>
                </div>
              </section>
              {docsContent}
